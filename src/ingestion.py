...
def load_cosa_markdown_data(directory_path):
    documents = []
    for root, dirs, files in os.walk(directory_path):
        category = os.path.basename(root)

        #We walk through the subfolders by category
        for file in files:
            if file.endswith('.md'):
...
# Loads structured JSONL data from COSA (City of San Antonio) emergency preparedness sources.
...
def load_jsonl_data(jsonl_path):
    ...
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            metadata = {
                "source_url": str(rec.get('source_url', 'https://www.sanantonio.gov')),
                "title": rec.get('title', ''),
